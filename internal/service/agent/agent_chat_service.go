...
func (c AgentService) agentTools(reqCtx RequestContext, emit AgentChatEmitFunc, knowledgeBaseIDs []string) map[string]AgentTool {
	...
	return map[string]AgentTool{
		...
		"clock_in_or_out":                  tool("clock_in_or_out", c.toolClockInOrOut),
		"my_attendance_anomalies":          tool("my_attendance_anomalies", c.toolMyAttendanceAnomalies),
		"team_attendance_today":            tool("team_attendance_today", c.toolTeamAttendanceToday),
		"team_attendance_anomalies":        tool("team_attendance_anomalies", c.toolTeamAttendanceAnomalies),
		"schedule_coverage_gaps":           tool("schedule_coverage_gaps", c.toolScheduleCoverageGaps),
		"schedule_notification":            tool("schedule_notification", c.toolScheduleNotification),
		"my_form_history":                  tool("my_form_history", c.toolMyFormHistory),
		"withdraw_or_cancel_leave_request": tool("withdraw_or_cancel_leave_request", c.toolWithdrawLeaveRequest),
		...
		"hr_attendance_anomaly_report":     tool("hr_attendance_anomaly_report", c.toolHRAttendanceAnomalyReport),
		"payroll_attendance_export":        tool("payroll_attendance_export", c.toolPayrollAttendanceExport),
		"hr_faq_insights":                  tool("hr_faq_insights", c.toolHRFAQInsights),
		"employee_bulk_import":             tool("employee_bulk_import", c.toolEmployeeBulkImport),
		"employee_lifecycle_change":        tool("employee_lifecycle_change", c.toolEmployeeLifecycleChange),
		"employee_change_history":          tool("employee_change_history", c.toolEmployeeChangeHistory),
		"create_hr_handoff_ticket":         tool("create_hr_handoff_ticket", c.toolCreateHRHandoffTicket),
...
func agentToolDescription(name string) string {
	descriptions := map[string]string{
		...
		"clock_in_or_out":                  "Prepare a geofenced clock-in or clock-out confirmation. Args: direction (in/out), latitude, longitude, accuracy_meters, optional device_id and client_event_id. Never clocks before confirmation.",
		"my_attendance_anomalies":          "Read current employee anomaly days. Optional args: month in YYYY-MM.",
		"team_attendance_today":            "Read authorized team attendance for today. Optional args: department_id, keyword, limit.",
		"team_attendance_anomalies":        "Read authorized team anomaly rows. Optional args: month, department_id, severity, page, page_size.",
		"schedule_coverage_gaps":           "Find scheduled employees without a complete normal attendance record today. Optional args: department_id, keyword, limit. Does not infer staffing demand targets.",
		"schedule_notification":            "Prepare attendance or schedule reminder notifications. Args: employee_ids, title, body, optional link_url. Never sends before confirmation.",
		...
		"withdraw_or_cancel_leave_request": "Prepare withdrawal of an owned leave form. Args: form_instance_id and optional reason. Never withdraws before confirmation.",
		...
		"hr_attendance_anomaly_report":     "Read a bounded HR anomaly report and optional overtime alerts. Optional args: month, department_id, severity, page, page_size, employee_page, employee_page_size, overtime_threshold_hours.",
		"payroll_attendance_export":        "Generate a bounded payroll attendance CSV. Optional args: month, department_id, kind (attendance/clock/abnormal). Returns base64 content.",
		"hr_faq_insights":                  "Read recent top prompts retained by employee-service Agent usage statistics. No args.",
		"employee_bulk_import":             "Validate and prepare import of 1-50 employee objects. Args: employees array. Never creates employees before confirmation.",
		"employee_lifecycle_change":        "Prepare an employee status transition. Args: employee_id, status, reason, start_date, optional end_date and details. Never changes status before confirmation.",
		"employee_change_history":          "Read one authorized employee's internal experiences and latest lifecycle transition. Args: employee_id.",
		"create_hr_handoff_ticket":         "Create an hr-handoff form draft and show its submission confirmation. Args: category, question, checked_sources, missing_information.",
