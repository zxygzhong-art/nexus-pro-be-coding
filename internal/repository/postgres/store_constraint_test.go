...
func TestIsExclusionConstraint(t *testing.T) {
	...
	if isExclusionConstraint(err, "another_constraint") {
		t.Fatal("unexpected match for another constraint")
	}
	if isExclusionConstraint(&pgconn.PgError{Code: "23505", ConstraintName: "attendance_shift_assignments_active_no_overlap"}, "attendance_shift_assignments_active_no_overlap") {
